...
# DSS analysis
##Multiple Univariate analysis - Including missing data
```{r Multiple Univariate analysis - Including missing data DSS}
surv_time <- "censor_time_OS"
...
```{r  Multivariate Survival Analysis - only complete cases DSS, fig.width=10, fig.height=8, warning=FALSE, message=FALSE, eval=TRUE}
# # To remove both (NAs and empty):
dataDF_complete <- dataDF %>%
  drop_na() %>%  # Remove NA values
  filter_all(all_vars(. != "")) %>%  # Remove empty strings
...
```{r Combined Univariable vs Multivariable Forest Plots - DSS, fig.width=10, fig.height=7, warning=FALSE, message=FALSE, eval=TRUE}
# Prepare empty list to collect rows
rows <- vector("list", 0)
...
```{r - 5 year KM curves whole cohort, fig.width=10, fig.height=8}
# OS plot
fit_OS <- survfit(
