...
saveWorkbook(
  wb,
  file = "Supplementary_Table_2.xlsx",
  overwrite = TRUE
)
```
...
```{r Multiple Univariate analysis - Including missing data - MSS}
surv_time <- "censor_time_OS"
...
```{r Multivariate Survival Analysis - only complete cases - MSS, fig.width=10, fig.height=8, warning=FALSE, message=FALSE, eval=TRUE}
surv_time <- "censor_time_OS"
surv_status <- "censor_status_MSS"
surv_time_label <- "OS"
# # To remove both (NAs and empty):
...
```{r Combined Univariable vs Multivariable Forest Plots - MSS, fig.width=14, fig.height=7, warning=FALSE, message=FALSE, eval=TRUE}
# Prepare empty list to collect rows
rows <- vector("list", 0)
