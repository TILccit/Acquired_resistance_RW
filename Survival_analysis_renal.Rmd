...
# RSS analysis
##Multiple Univariate analysis - Including missing data
```{r Multiple Univariate analysis - Including missing data RSS}
surv_time <- "censor_time_OS"
...
```{r  Multivariate Survival Analysis - only complete cases RSS, fig.width=10, fig.height=8, warning=FALSE, message=FALSE, eval=TRUE}
# # To remove both (NAs and empty):
dataDF_complete <- dataDF %>%
  drop_na() %>%  # Remove NA values
  filter_all(all_vars(. != "")) %>%  # Remove empty strings
...
```{r Combined Univariable vs Multivariable Forest Plots - RSS, fig.width=10, fig.height=7, warning=FALSE, message=FALSE, eval=TRUE}
# Prepare empty list to collect rows
rows <- vector("list", 0)
