...
```{r PSM using 2:1 or 3:1, warning=F, message=F,eval=TRUE,  fig.width=25, fig.height=10}
ratio = 2
...
```{r power calculation}
results_power <- lapply(PFS_psm_all$matched_dfs, function(mdf) {
  posthoc_power_from_matched(mdf, group_var = "Tumor",
                             timevar = "PFS_months", statusvar = "Progressed", alpha = 0.05)
})

for (i in seq_along(results_power)) {
  cat("Comparison:", names(results_power)[i], "\n")
  print(results_power[[i]][c("n1","n2","D","HR","observed_power","required_events_80","required_events_90")])
  cat("\n")
}
```
...
posthoc_power_twoarm <- function(df,
                                 group_var   = "DOR",
  ...
  # restrict to complete cases for the vars we actually use
  vars_needed <- c(group_var, timevar, statusvar)
  if (!is.null(strata_var)) vars_needed <- c(vars_needed, strata_var)
  df <- df[complete.cases(df[, vars_needed]), ]

  g <- droplevels(as.factor(df[[group_var]]))
  if (length(levels(g)) != 2L)
    stop("Need exactly 2 levels in ", group_var, " (got: ", paste(levels(g), collapse = ", "), ")")

  # allocation ratio
  tab <- table(g)
  n1  <- as.integer(tab[1])   # reference level
  n2  <- as.integer(tab[2])
  p   <- n1 / (n1 + n2)       # proportion in group 1

  # total number of events
  D <- sum(df[[statusvar]] == 1L, na.rm = TRUE)

  # Cox model formula
  fml <- if (is.null(strata_var)) {
    as.formula(sprintf("Surv(%s, %s) ~ %s", timevar, statusvar, group_var))
  } else {
    as.formula(sprintf("Surv(%s, %s) ~ %s + strata(%s)",
                       timevar, statusvar, group_var, strata_var))
  }
  ...
  # Schoenfeld-based post-hoc power and required events
  power_obs <- schoenfeld_power(D = D, p = p, HR = HR, alpha = alpha)
  req80     <- schoenfeld_required_events(target_power = 0.80, p = p, HR = HR, alpha = alpha)
  req90     <- schoenfeld_required_events(target_power = 0.90, p = p, HR = HR, alpha = alpha)
...
# Make sure DOR is coded with CR as reference, and PFS censoring has been set up
dataDF$DOR   <- relevel(dataDF$DOR, ref = "CR")
dataDF$Tumor <- droplevels(dataDF$Tumor)

power_overall_PR_CR <- posthoc_power_twoarm(
  df         = dataDF,
  group_var  = "DOR",               # CR vs PR
  timevar    = "censor_time_PFS",
  statusvar  = "censor_status_PFS",
  strata_var = "Tumor",             # as in a Cox: Surv ~ DOR + strata(Tumor)
  alpha      = 0.05
)

# Compact printout for the manuscript / reply letter
power_overall_PR_CR[c("n1","n2","D","HR","observed_power","required_events_80","required_events_90")]
